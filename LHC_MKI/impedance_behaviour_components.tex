...
\caption{The geometries simulated for the various components in the LHC-MKI. These are c-core ferrite only \subref{fig:mki-buildup-ferrite}, c-core ferrite and the ceramic tube \subref{fig:mki-buildup-ferrite-ceramic}, c-core ferrite with the ceramic tube containing 24 screen conductors \subref{fig:mki-buildup-ferrite-ceramic-cond} and finally the complete MKI magnet (without vacuum tank for clarity) \subref{fig:mki-buildup-ferrite-ceramic-cond-full}.}
...
\begin{center}
\subfigure[]{
\includegraphics[width=0.8\textwidth]{LHC_MKI/figures/mki-build-up-real-imp.pdf}
\label{fig:mki-buildup-real-imp}
}\\
\subfigure[]{
\includegraphics[width=0.8\textwidth]{LHC_MKI/figures/mki-build-up-imag-imp.pdf}
\label{fig:mki-buildup-imag-imp}
}
\end{center}
\caption{The \subref{fig:mki-buildup-real-imp} real component and the \subref{fig:mki-buildup-imag-imp} imaginary components of the LHC MKI kicker magnet impedances for different components in the magnet.}
...
\caption{Beam screens with different numbers of screen conductors removed from the design quantity of 24. Models of 15 \subref {fig:15-cond-together}, 19 \subref{fig:19-cond-together} and 24 \subref{fig:24-cond-together} screen conductors are considered for the impedance simulations. Conductors surrounded by the boxes are removed in this case.}
...
\caption{Beam screens with different numbers of screen conductors removed from the design quantity of 24. Models of 17 \subref {fig:17-cond-alt}, 19 \subref{fig:19-cond-alt}, 20 \subref{fig:20-cond-alt} and 24 \subref{fig:24-cond-alt} screen conductors are considered for the impedance simulations. Conductors that are coloured red are the removed in each case.}
...
\begin{center}
\subfigure[]{
\includegraphics[width=0.7\textwidth]{LHC_MKI/figures/mki-no-screen-conductors-real-impedance.pdf}
\label{fig:remove-cond-real-imp}
}\\
\subfigure[]{
\includegraphics[width=0.7\textwidth]{LHC_MKI/figures/mki-no-screen-conductors-imag-impedance.pdf}
\label{fig:remove-cond-imag-imp}
}
\end{center}
\caption{The longitudinal impedance of the MKIs with different numbers of screen conductors included in the beam screen. Shown is the real component \subref{fig:remove-cond-real-imp} and the imaginary component \subref{fig:remove-cond-imag-imp} of the longitudinal impedance.}
...
\end{center}
\caption{A comparison of the mesh that may be generated in CST Particle Studio using both the full model \subref{fig:mesh-full} and the cutdown model \subref{fig:mesh-cutdown}. Note the greatly increased mesh density in the important overlap region between the screen conductors and external metallization. The simulation time for the first is on the order a of day, for the second 30 minutes.}
...
\begin{center}
\subfigure[]{
\includegraphics[width=0.7\textwidth]{LHC_MKI/figures/mki-overlap-len-real-imp.pdf}
\label{fig:overlap-imp}
}\\
\subfigure[]{
\includegraphics[width=0.7\textwidth]{LHC_MKI/figures/mki-overlap-len-real-imp-zoom.pdf}
\label{fig:overlap-imp-zoom}
}
\end{center}
\caption{\subref{fig:overlap-imp} The simulated real component of the longitudinal beam coupling impedance for different lengths of the overlap, and \subref{fig:overlap-imp-zoom} a zoomed in plot of the first resonances of all the overlaps with the calculated resonance frequency (shown as vertical lines) for each based on the effective length. For each the thickness of the ceramic tube is 7mm.}
